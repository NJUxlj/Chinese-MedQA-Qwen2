...
def log_print(ctn: Any):
    ...
    该函数会自动获取调用者的信息（时间、文件名、行号、函数名），
    并将这些信息与要打印的内容一起格式化输出，便于调试和追踪代码执行位置
    
    Args:
        ctn: Any - 要打印的内容，可以是任意类型
    """
    current_time = datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")

    # 获取调用此函数的栈帧信息
    frame = inspect.currentframe().f_back
    # 从栈帧中提取调用者信息：函数名、行号、文件名
    function_name = frame.f_code.co_name
    line_number = frame.f_lineno
    file_name = frame.f_code.co_filename.split("/")[-1]
    
    # 格式化输出：[时间-文件名:行号:函数名]: 内容
...
def _timer(name: str, timing_raw: Dict[str, float]):
    ...
    这是一个生成器函数，用于测量代码块的执行时间。
    使用 Timer 上下文管理器来精确计时，并将结果累加到指定的字典中
    
    Args:
        name (str): 计时测量的名称/标识符
        timing_raw (Dict[str, float]): 存储计时信息的字典
    """
    # 使用 Timer 上下文管理器进行计时
    # Timer 会自动记录进入和退出上下文的时间差
    with Timer(name=name, logger=None) as timer:
        # yield 将控制权交给调用者，执行被计时的代码块
        # 当代码块执行完毕后，控制权会返回到这里
        yield
